...
where student_id = 1289 and c.course_id not in
(Select p.course_id
From Student s
Inner Join User u on u.user_id = s.student_id
Inner Join D_Member dm on u.user_id = dm.user_id
...
Inner Join Course c on c.course_id = cu.course_id
Inner Join PreReq p on c.course_id = p.course_id
where student_id = 1289 AND p.req_id in
(Select t.course_id
From Student s
Inner Join Takes t on t.s_id = s.student_id
where s.student_id = 1289 and t.letter_grade is not null and t.letter_grade NOT LIKE 'F%'))) as c
...
From Student s
Inner Join Takes t on t.s_id = s.student_id
where s.student_id = 1289 and t.letter_grade is not null and t.letter_grade NOT LIKE 'F%');
...
Inner Join Curriculum cu on c.course_id = cu.course_id
where s.student_id = 1289 and t.letter_grade is not null and t.letter_grade NOT LIKE 'F%')
UNION
(Select p.course_id, c.course_code, c.name, cu.course_type
From Student s
...
Inner Join Course c on c.course_id = cu.course_id
Inner Join PreReq p on c.course_id = p.course_id
where student_id = 1289 AND p.req_id NOT in
(Select t.course_id
From Student s
Inner Join Takes t on t.s_id = s.student_id
where s.student_id = 1289 and t.letter_grade is not null and t.letter_grade NOT LIKE 'F%'))) as t;
